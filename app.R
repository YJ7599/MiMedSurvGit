...
list.of.packages <- c('shiny', 'rmarkdown', 'seqinr', 'shinydashboard', 'fresh', 'dashboardthemes', 'plotly', 'shinyWidgets', 'shinyjs', 'googleVis', 'xtable', 'rgl',
                      'DT', 'htmltools', 'biomformat', 'phangorn',  'zip', 'dplyr', 'forestplot', 'fossil', 'picante',
...

{
  ...
  INPUT_INDIVIDUAL_DATA_COMMENT2 = p("You can download example microbiome data 'Individual.zip'. This zip file contains four data components, feature
  ...
  DATA_TRANSFORM_COMMENT = p("Normalize the data into the CLR (centered log ratio) (Aitchison, 1982), arcsine-root, proportion, and rarefied count (Sanders, 1968) formats for each taxonomic rank (phylum, class, order, family, genus, species).")
  DATA_TRANSFORM_REFERENCE = p("1. Aitchison J. The statistical analysis of compositional data. J R Statist Soc B. 1982;44:2:139-77", br(), 
...
server <- function(input, output, session) {
  ...
  otu.tab <- otu_table(FTMP)
  tax.tab <- tax_table(FTMP)
  tree <- phy_tree(FTMP)
  sam.dat <-  as.data.frame(as(sample_data(FTMP), "data.frame"))
    ...
    observe({
      if (input$inputOption == "Phyloseq") {
        ...
        output$addDownloadinfo <- renderUI({
          tagList(
            box(title = strong("Example Data", style = "color:white"), width = NULL, status = "primary", solidHeader = TRUE,
                downloadButton("downloadData.1.Ind", "Individual.zip", width = '30%', 
                               style="color: #000000; background-color: #FFFFFF; border-color: #ff7f50"),
  ...
  QC = observe(suspended = T,{
    ...
    kingdom_options <- names(table(tax_table(infile$biom)[,1]))[table(tax_table(infile$biom)[,1])>30]
    
    output$kingdom_sel <- renderUI({selectInput("kingdom", h4(strong("Kingdom")), choices = kingdom_options, selected = kingdom_options[1])}) 
  ...
  observeEvent(chooseData$sam.dat, {
    sam_dat_check <<- chooseData$sam.dat
    
    mon.sin.rev.bin.con <-  suppressWarnings({is.mon.sin.rev.bin.con(chooseData$sam.dat)})
    out_taxa_options <- try(surv.pri.func(chooseData$sam.dat, mon.sin.rev.bin.con)[[2]], silent = TRUE) 
    censor_taxa_options <- try(surv.pri.func(chooseData$sam.dat, mon.sin.rev.bin.con)[[1]], silent = TRUE)
  ...
  observeEvent(chooseData$alpha.div, {
    ...
    ##### ALPHA DIVERSITY #####
    mon.sin.rev.bin.con <- suppressWarnings({is.mon.sin.rev.bin.con(chooseData$sam.dat)})
    out_taxa_options <- surv.pri.func(chooseData$sam.dat, mon.sin.rev.bin.con)[[2]]
    censor_taxa_options <- surv.pri.func(chooseData$sam.dat, mon.sin.rev.bin.con)[[1]]
    ...
    output$taxa_outcome <- renderUI({
      tagList(
        prettyRadioButtons("dataType_taxa", label = h4(strong("Data Format?", style = "color:black")), animation = "jelly",
                           c("CLR (default)", "Arcsine-root","Proportion",  "Count (rarefied)"), selected = "CLR (default)",width = '70%'),
  ...
  observeEvent(input$run, {
    withProgress(
      ...
      detail = 'This may take a while...', value = 0, {
        ...
        chooseData$mon.sin.rev.bin.con = suppressWarnings({is.mon.sin.rev.bin.con(chooseData$sam.dat)})
        ...
        output$divCalcDownload <- renderUI({
          tagList(
            box(title = strong("Download Data", style = "color:white"), width = NULL, status = "primary", solidHeader = TRUE,
                ...
                h5("Arcsine-root"),
                downloadButton("taxadataArc", "Download", width = '50%', style = "color:black; background-color: red3"), br(),
                
                h5("Proportion"), 
                downloadButton("taxadataProp", "Download", width = '50%', style = "color:black; background-color: red3"), br(), 
                
                h5("Count (rarefied)"),
                downloadButton("taxadataCount", "Download", width = '50%', style = "color:black; background-color: red3"), br(),br()
            )
          )
        })
    ...
    withProgress({
      ...
      if (input$non.covariate == "None") {
        ...
        if (input$non.chooseMethod == "Cox model") {
          ...
          surv.KM.fit.check <<- surv.KM.fit
          surv.dat.check <<- surv.dat 
          non.test.check <<- input$non.test
          
          output$surv.plot3 = suppressWarnings(renderPlot({

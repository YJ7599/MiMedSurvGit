...
server <- function(input, output, session) {
  ...
  observeEvent(chooseData$alpha.div, {
            ...
            if (input$interaction == "Yes (Default)"){
              ...
              output$chooseMethodType <- renderUI({
                tagList(
                  ...
                  p(" ", style = "margin-bottom: +15px;"),
                  prettyRadioButtons("chooseMethod",label = NULL, icon = icon("check"),
                                     animation = "jelly", c("Imai Method (Default)", "DACT"), 
    ...
    output$long.treatvar <- renderUI({
      tagList(
        h4(strong("Treatment variable", style = "color:black")),
        p(" ", style = "margin-bottom: +15px;"),
        
        selectInput("long.treatvar", label = NULL,
    ...
    observeEvent(input$long.treatvar, {
      output$long.outvars <- renderUI({
        tagList(
          p(" ", style = "margin-bottom: +20px;"),
          h4(strong("Outcome variable", style = "color:black")),
          p(" ", style = "margin-bottom: +15px;"),
          selectInput("long.outvar", label = NULL,
                      c("Choose one" = "", select.outcome.func(chooseData$sam.dat, chooseData$treat_vars, input$long.treatvar)), "gingival.index.bin", width = '80%'),
          p(" ", style = "margin-bottom: -10px;"),
          p("Health or disease outcomes.", style = "font-size:10pt"),
        )
      })
      
      observeEvent(input$long.outvar, {
        
        output$long.cluster <- renderUI({
          tagList(
            p(" ", style = "margin-top: 25px;"),
            h4(strong("Cluster variable", style = "color:black")),
            p(" ", style = "margin-bottom: +15px;"),
            selectInput("long.cluster", label = NULL,
                        c("Choose one" = "", select.outcome.func(chooseData$sam.dat, colnames(chooseData$sam.dat), c(input$long.treatvar, input$long.outvar))), "vapors", width = '80%'),
            p(" ", style = "margin-bottom: -10px;"),
      ...
      observeEvent(input$long.cluster, {
        output$long.timept <- renderUI({ 
          tagList(
            p(" ", style = "margin-top: 25px"),
            h4(strong("Timepoint variable"), style = "color:black"), 
            p(" ", style = "margin-bottom: +15px"),
            selectInput("long.timept", label = NULL, 
                        c("Choose one" = "", select.outcome.func(chooseData$sam.dat, colnames(chooseData$sam.dat), c(input$long.treatvar, input$long.outvar, input$long.cluster))), "time.pt", width = "80%")
          )})
      })
    })
    
    output$long.outcome_types <- renderUI({
      tagList(
        uiOutput("long.interactions"),
        uiOutput("long.covariates"),
        uiOutput("long.chooseMethodType"),
        actionButton("long.runbtn_alpha", (strong("Run!")), class = "btn-info",
                     style="color: #000000; background-color: #FFFFFF; border-color: #ff7f50")
      )
    })
    ...
    observeEvent(input$long.treatvar, {
      observeEvent(input$long.outvar, {
        observeEvent(input$long.cluster, {
          ...
          if (input$long.outvar %in% chooseData$out_vars) {
            
            ## Interaction UI 
            output$long.interactions <- renderUI({
              tagList(
                p(" ", style = "margin-top: 25px;"),
                h4(strong("Interaction term", style = "color:black")),
                p(" ", style = "margin-bottom: +15px;"),
                prettyRadioButtons("long.interaction", label = NULL, 
                                   icon = icon("check"),
                                   animation = "jelly", c("Yes (Default)", "No"), selected = "Yes (Default)", width = '80%'),
                p(" ", style = "margin-bottom: -10px;"),
                p("To include (yes) or exclude (no) an interaction term between a treatment and a mediator (microbiome).",
            ...
            ntselected.vars = select.covariates.func.surv(chooseData$sam.dat, chooseData$mon.sin.rev.bin.con, input$long.treatvar, input$long.outvar, input$long.cluster)
            is.results$outResult = is.bin.con.pri(chooseData$sam.dat, chooseData$mon.sin.rev.bin.con, input$long.outvar)
            
            ## Covariates UI
            output$long.covariates <- renderUI({
              tagList(
                p(" ", style = "margin-top: 25px;"),
                h4(strong("Covariate(s)", style = "color:black")),
                p(" ", style = "margin-bottom: +15px;"),
                prettyRadioButtons("long.covariate",label = NULL, 
                                   icon = icon("check"),
                                   animation = "jelly", c("None", "Covariate(s)"), selected = "None", width = '70%'),
                p(" ", style = "margin-bottom: -10px;"),
                p("Potential confounders (e.g., age, gender) to be adjusted for.", style = "font-size:10pt"),
                ...
                shinyjs::hidden(
                  shiny::div(id = "long.covariates_variables", style = "margin-left: 2%",
                             prettyCheckboxGroup("long.covariateOptions"," Please select covariate(s)", 
                                                 choices = ntselected.vars, width = '70%'))))
            })
            
            ## Covariates Effect
            observeEvent(input$long.covariate,{
              if (input$long.covariate == "Covariate(s)") {
                shinyjs::show("covariates_variables")
              } 
              else if (input$long.covariate == "None") {
                shinyjs::hide("covariates_variables")
              }
            })
            
            ## Mediation Analysis Type UI
            output$long.chooseMethodType <- renderUI({
              tagList(
                p(" ", style = "margin-top: 25px;"),
                h4(strong("Analytic method", style = "color:black")),
                p(" ", style = "margin-bottom: +15px;"),
                prettyRadioButtons("long.chooseMethod",label = NULL, icon = icon("check"),
                                   animation = "jelly", c("Imai Method"), 
                                   selected = "Imai Method", width = '70%'))
            })
          }
    ...
    withProgress(
      ...
      detail = 'This may take a while...', value = 0, {
        ...
        if (input$chooseMethod %in% c("Imai Method (Default)", "Imai Method")) {
          ...
          Regression_result <<- tryCatch(Regression(Data.na.stand, Mediator.list, Treatment, Event, Outcome, Is.covariates, Covariates, 
                                          "FALSE", Treatment.type), 
                                        error = function(e) {
                                          message("No outcome is available!")
                                          showModal(modalDialog(div("No outcome is available!")))
                                          return(NULL)})
            ...
            fluidRow(style = "position:relative",
                     tabBox(width = 12, title = strong("Forest Plot", style = "color:black"),
                            tabPanel("Divide-Aggregate Composite-Null Test", align = "center",
                                     plotOutput("DACT_forestplot", height = 500, width = 800))
                            ))
          })
  ...
  observeEvent(input$long.runbtn_alpha,{
    
    shinyjs::disable("long.runbtn_alpha") 
    shinyjs::disable("long.treatvar")
    shinyjs::disable("long.outvars")
    shinyjs::disable("long.cluster")
    shinyjs::disable("long.timept")
    shinyjs::disable("long.outcome_types")
    ...
    withProgress(
      ...
      detail = 'This may take a while...', value = 0, {
        ...
        Treatment = input$long.treatvar
        Cluster = input$long.cluster
        Outcome = input$long.outvar
        Time = input$long.timept 
        Interaction = ifelse(input$long.interaction == "Yes (Default)", "TRUE", "FALSE")
        Is.covariates = input$long.covariate
        Covariates = input$long.covariateOptions
        Covariates.regression = ifelse(is.null(input$long.covariateOptions), NA, paste0(input$long.covariateOptions, collapse = " + "))
        Method = input$long.chooseMethod
        ...
        if (input$long.chooseMethod %in% c("Imai Method (Default)", "Imai Method")) {
          
          ## Data Manipulation
          if (Is.covariates == "Covariate(s)") {
            for (i in 1:length(input$long.covariateOptions)) {
              if (is.bin.con.pri(chooseData$sam.dat, chooseData$mon.sin.rev.bin.con, input$long.covariateOptions[i]) == "Binary") {
                Data.na.stand[[input$long.covariateOptions[i]]] <- as.factor(Data.na.stand[[input$long.covariateOptions[i]]])
              } 
              else if (is.bin.con.pri(chooseData$sam.dat, chooseData$mon.sin.rev.bin.con, input$long.covariateOptions[i]) == "Continuous") {
                Data.na.stand[[input$long.covariateOptions[i]]] <- as.numeric(Data.na.stand[[input$long.covariateOptions[i]]])
              }
            }
          }
          
          Treatment.type <<- is.bin.con.pri(chooseData$sam.dat, chooseData$mon.sin.rev.bin.con, input$long.treatvar)
          Outcome.type <<- is.bin.con.pri(chooseData$sam.dat, chooseData$mon.sin.rev.bin.con, input$long.outvar)
          
          Med.out <- tryCatch(Mediation.Long(Data.na.stand, Mediator.list,
                                        Treatment, Outcome, Cluster, Time, Is.covariates, Covariates.regression,
                                        Interaction, Treatment.type, Outcome.type, 3000), error = function(e) {
                                message("No outcome is available!")
                                showModal(modalDialog(div("No outcome is available!")))
                                return(NULL)})

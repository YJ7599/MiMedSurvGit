...
surv.KM.fit.func <- function( surv.dat ) {
  suppressWarnings({survfit( Surv( survtime, status )~treat, data = surv.dat, conf.type = c("plain"), type = 'kaplan-meier')}) 
}
...
surv.KM.plot.func <- function( surv.KM.fit, surv.dat, input = "Log-rank test", follow.start, follow.end ) {
  ...
  if ( input != "Cox PH Model"){
    ggsurv <- suppressWarnings({ggsurvplot( surv.KM.fit,
                          data = surv.dat,
    ...
    ggsurv$plot <- suppressWarnings({ggsurv$plot + ggplot2::annotate("text",
                        x = 8, y = 0.1, 
                        label = output, size = 4)}) 
  }
  else {
    ggsurv <- suppressWarnings({ggsurvplot( survfit(surv.KM.fit), data = surv.dat,
                          color = "#2E9FDF",

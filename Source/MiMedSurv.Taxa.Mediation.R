...
tax.trans <- function(otu.tab, tax.tab, rare.otu.tab, rare.tax.tab, sub.com = TRUE, na.code = "NANANA") {
  ...
  for (j in 1:6) {
    ...
    if(length(tax.imp.prop) == 0) {
      ...
      for (i in 1:length(lib.size)) {
        tax.imp.prop[i,] <- (tax.count[i,]+0.01)/lib.size[i]
      }
  ...
  for (i in 2:(5+inc)){
    ...
    for (k in 1:length(taxa.med)){
      ...
      for (j in uniq_list_ind){
        ...
        if (length(new_list[[i-1]][[k]][which(uniq.list.last),]) != 0){
...
mediation.taxon.total <- function(sam.dat, taxa, taxa_unique, 
  ...
  for (i in 1:(5+inc)){
    ...
    if (sum(is.na(taxa.uniq.pos)) == 1){
      
      if(treat.type == "binary"){
        result <<- mediation.taxon.ind(sam.dat, taxon.med, NULL, exposure, covariates, 
                                  outcome, event, interaction = interac, n.sim = n.sim, inc = inc, rank = i)
        
        list_total[[i]] <- result$list_binary
        lets_see[[i]] <- result$med_out
        
      }else if(treat.type == "continuous"){
        list_total[[i]] <- mediation.taxon.ind(sam.dat, taxon.med, NULL, exposure, covariates, 
                                               outcome, event, interaction = interac,  n.sim = n.sim, inc = inc, rank = i)$list_continuous
      }
    }else{
      if(treat.type == "binary"){
        result <<- mediation.taxon.ind(sam.dat, taxon.med, taxa.uniq.pos, exposure, covariates, 
                                  outcome, event, interaction = interac, n.sim = n.sim, inc = inc, rank = i)
...
mediation.taxon.ind <- function(sam.dat, taxon_med, taxa_uniq_name, 
                                exposure, covariates, 
                                outcome, event, interaction = TRUE, 
                                n.sim = 10, inc = FALSE, rank){
    ...
    for (med in mediator.list){
      ...
      if(med %in% colnames(taxon.med)[taxa_uniq_name]){
        ...
        if (length(covariates) != 0){
          ...
          dat$surv.var <- Surv(time = sam.dat[[outcome]], event = sam.dat[[event]])
          surv.var <- "surv.var"
          
          if (interaction == TRUE){
            
            f2 <<- as.formula(paste(surv.var, "~", med, "*", exposure, "+", paste(covariates, collapse = "+"), sep = " "))
          }else{
            f2 <<- as.formula(paste(surv.var, "~", med, "+", exposure, "+", paste(covariates, collapse = "+"), sep = " "))
          }
        }else{
          ...
          if (interaction == TRUE){
            f2 <<- as.formula(paste(surv.var, "~", med, "*", exposure, sep = " "))
          }else{
            f2 <<- as.formula(paste(surv.var, "~", med, "+", exposure, sep= " "))
          }
        }
  ...
  if(treat.type == "binary"){
    list.binary <- list(acme_average = acme.average, acme_control = acme.control, acme_treated = acme.treated, ade_average = ade.average, total_effect = total)
    ...
    list.continuous <- list(acme_average = acme.average, ade_average = ade.average, total_effect = total)
...
result_med_out <- function(sam.dat, taxa, treat, out, model = "med", interac){
  final_list <- list() 
...
result_med_out_dact <- function (sam.dat, taxa, taxa_unique, exposure, outcome, event, covariates, reg = "linear", interac) {
  final_list_med <- list() 
  final_list_out <- list() 
  
  for (i in 1:6){
    mat_med <- matrix(NA, nrow = length(taxa[[i]]), ncol = 6, dimnames = list(names(taxa[[i]]), c("Effect", "SE", "P.val", "Q.val", "Lower", "Upper")))
    mat_out <- matrix(NA, nrow = length(taxa[[i]]), ncol = 6, dimnames = list(names(taxa[[i]]), c("Effect", "HR", "P.val", "Q.val", "Lower", "Upper")))
    
    for (k in 1:length(taxa[[i]])){
      mv <- taxa[[i]][[k]]
      data_mani <- data.frame(exposure = sam.dat[, exposure], outcome = sam.dat[, outcome], med = mv)
      data_mani$surv.var <- Surv(time = sam.dat[[outcome]], event = sam.dat[[event]])
      surv.var <- "surv.var"
      ...
      out.fit <- coxph(formula = f2, data = data_mani)
      ...
      est_out <- result.out$coefficients[1,1]
      hr_out <- result.out$conf.int[1, 1]
      pval_out <- result.out$coefficients[1, 5]
      conf.int_lower <- result.out$conf.int[1, 3]
      conf.int_upper <- result.out$conf.int[1, 4]
      
      mat_out[k, ] <- c(est_out, hr_out, pval_out, NA, conf.int_lower, conf.int_upper) 
    }
    
    mat_med[, "Q.val"] <- p.adjust(mat_med[,3], "BH")
    mat_out[, "Q.val"] <- p.adjust(mat_out[,3], "BH")
...
# 
# sobel_result <- result_dact_new 
# sobel_med <- result_dact_med_new 
# sobel_out <- result-dact_out_new 
# taxa.names.out <- taxa.names.rank(taxa, include)
# species.include <- include 
# report.type <- "Est"
# mult.test.cor <- TRUE 
...
taxa.forest.plot.pages2_sobel <- function(page.taxa.q.out, page, model) {
  ...
  if(length(page.taxa.q.out$all.text.tab_1) == 0){
    ...
    if (model == "med"){
      text.tab <- page.taxa.q.out$all.text.tab_1[[page]]
      ci <- page.taxa.q.out$all.ci.tab_1[[page]]
      ...
      text.tab[,3] <- substr(text.tab[,3], 1, 55)
      
      new_text <- cbind(text.tab,  ci)
      new_text <- data.frame(new_text) 
      
      colnames(new_text) <- c("id", "rank", "taxon", "pval", "qval", "eff_med", "mean", "lower", "upper")
      ...
      forestplot(new_text[, c("id", "rank", "taxon", "pval", "qval", "eff_med")], mean = as.numeric(new_text$mean), lower = as.numeric(new_text$lower), upper = as.numeric(new_text$upper), hzrl_lines = TRUE, is.summary  = c(TRUE, rep(FALSE, nrow(new_text)-1)), new_page=TRUE, boxsize=0.18, grid=0, colgap = unit(0.6, "cm"), graphwidth = unit(8.5, "cm"), lineheight = "lines", line.margin = unit(0.16, "cm"), col=fpColors(box=rgb(1,0,0,0.5), line="black", summary="red3"), xlab="95% Confidence Interval", mar = unit(c(0.5,0,0.5,0), "cm"), txt_gp=fpTxtGp(label=list(gpar(fontfamily="", cex=0.9), gpar(fontfamily="", cex=0.9)), ticks=gpar(fontfamily="", cex=0.9), xlab=gpar(fontfamily="", cex=0.9)), shapes_gp = styles)
      
    }else{
      text.tab <- cbind(forest.data_med$all.text.tab_1[[1]][, -6], page.taxa.q.out$all.text.tab_2[[page]])
      ci <- page.taxa.q.out$all.ci.tab_2[[page]]
      
      plot.new()
      
      str.max <- lapply(page.taxa.q.out$all.text.tab_2, nchar)
      for(i in 1:length(page.taxa.q.out$all.text.tab_2)){
        str.max[[i]] <- str.max[[i]][,3]
      }
      ...
      colnames(new_text) <- c("id", "rank", "taxon", "pval", "qval", "eff_out", "mean", "lower", "upper")
      ...
      forestplot(new_text[, c("id", "rank", "taxon", "pval", "qval", "eff_out")], mean = as.numeric(new_text$mean), lower = as.numeric(new_text$lower), upper = as.numeric(new_text$upper), hzrl_lines = TRUE, is.summary  = c(TRUE, rep(FALSE, nrow(new_text)-1)), new_page=TRUE, boxsize=0.18, grid=0, colgap = unit(0.6, "cm"), graphwidth = unit(8.5, "cm"), lineheight = "lines", line.margin = unit(0.16, "cm"), col=fpColors(box=rgb(1,0,0,0.5), line="black", summary="red3"), xlab="95% Confidence Interval", mar = unit(c(0.5,0,0.5,0), "cm"), txt_gp=fpTxtGp(label=list(gpar(fontfamily="", cex=0.9), gpar(fontfamily="", cex=0.9)), ticks=gpar(fontfamily="", cex=0.9), xlab=gpar(fontfamily="", cex=0.9)), shapes_gp = styles)
    }
...
### Replace tax.trans with tax.trans.subcominv

tax.trans.subcominv <- function(otu.tab, tax.tab, rare.otu.tab, rare.tax.tab, na.code = "NANANA") {
  
  n <- ncol(otu.tab)
  lib.size <- colSums(otu.tab)
  otu.tab.imp.count <- otu.tab + 0.1
  lib.size.imp <- colSums(otu.tab.imp.count)
  rare.n <- ncol(rare.otu.tab)
  ...
  for (j in 1:6) {
    ...
    tax.imp.count <- matrix(NA, n, length(tax))
    for (i in 1:length(tax)) {
      ind.tax <- which(tax.tab[,j+1] == tax[i])
      tax.imp.count[,i] <- colSums(otu.tab.imp.count[ind.tax,])
    }
    rownames(tax.imp.count) <- colnames(otu.tab.imp.count)
    colnames(tax.imp.count) <- tax
    
    tax.imp.prop <- matrix(NA, n, length(tax))
    for (i in 1:length(lib.size.imp)) {
      tax.imp.prop[i,] <- tax.imp.count[i,]/lib.size.imp[i]
    }
    rownames(tax.imp.prop) <- colnames(otu.tab.imp.count)
    colnames(tax.imp.prop) <- tax

...
Mediation.Long = function(data.na, mediator.list,
                     treatment, outcome, cluster, time, is.covariates, covariates,
                     interaction = c("TRUE", "FALSE"),
                     treatment.type = c("Binary", "Continuous"), outcome.type = c("Binary", "Continuous"), n.sim) {
  ...
  for (med in mediator.list) {
    ...
    length_time_pt <- length(table(data.na[[time]]))
    
    if (length_time_pt < 3){
      if (is.covariates == "None") {
        
        f1 <<- as.formula(paste(med, "~", treatment, "+(1|", cluster , ")" , sep=" "))
        Med.fit <- lmer(f1, data = data.na)
        
        if (interaction == "TRUE") {
          f2 <<- as.formula(paste(outcome, "~", med, "*", treatment, "+(1|", cluster , ")", sep=" "))
        }
        else if (interaction == "FALSE"){
          f2 <<- as.formula(paste(outcome, "~", med, "+", treatment, "+(1|", cluster , ")", sep=" "))
        }
        ...
        f1 <<- as.formula(paste(med, "~", treatment, "+", covariates, "+(1|", cluster , ")",  sep=" "))
        Med.fit <- lmer(f1, data = data.na)
        
        if (interaction == "TRUE") {
          f2 <<- as.formula(paste(outcome, "~", med, "*", treatment, "+", covariates, "+(1|", cluster , ")", sep=" "))
        }
        else if (interaction == "FALSE"){
          f2 <<- as.formula(paste(outcome, "~", med, "+", treatment, "+", covariates, "+(1|", cluster , ")", sep=" "))
        }
    ...
    }else if(length_time_pt > 2){
      if (is.covariates == "None") {
        f1 <<- as.formula(paste(med, "~", treatment, "+(1| +", time, "|", cluster , ")", sep=" "))
        Med.fit <- lmer(f1, data = data.na)
        
        if (interaction == "TRUE") {
          f2 <<- as.formula(paste(outcome, "~", med, "*", treatment, "+(1|", time, "|", cluster , ")", sep=" "))
        }
        else if (interaction == "FALSE"){
          f2 <<- as.formula(paste(outcome, "~", med, "+", treatment, "+(1|", time, "|", cluster , ")", sep=" "))
        }
        ...
        f1 <<- as.formula(paste(med, "~", treatment, "+", covariates, "+(1|", time, "|", cluster , ")", sep=" "))
        Med.fit <- lmer(f1, data = data.na)
        
        if (interaction == "TRUE") {
          f2 <<- as.formula(paste(outcome, "~", med, "*", treatment, "+", covariates, "+(1|", time, "|", cluster , ")", sep=" "))
        }
        else if (interaction == "FALSE"){
          f2 <<- as.formula(paste(outcome, "~", med, "+", treatment, "+", covariates, "+(1|", time, "|", cluster , ")", sep=" "))
        }
    ...
    if(outcome.type == "Binary"){
      Out.fit <- glmer(f2, family = binomial(link = "probit"), data = data.na)
      Med.out[[med]] <- mediate(Med.fit, Out.fit, treat = treatment, mediator = med, sims = n.sim)
    }else{
      Out.fit <- lmer(f2, data = data.na)
      Med.out[[med]] <- mediate(Med.fit, Out.fit, treat = treatment, mediator = med, sims = n.sim)
    }
...
Sensitivity_analysis <- function(med.out, n.sims) {
